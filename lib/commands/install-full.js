...
//
// Sources skills from <cwd>/_vibekitvn/ (extracted by `vibekit-vn activate`).
// Two modes:
...
export async function installFull(options = {}) {
  ...
  if (!bundleExists(targetDir)) {
    ...
    const msg = lic
      ? `Skill content chưa được tải về. Chạy: ${ui.color.cyan('vibekit-vn activate ' + lic.license_key)}`
      : `Chưa activate license. Chạy: ${ui.color.cyan('vibekit-vn activate <KEY>')}\n\nMua key qua: ${ui.color.cyan('https://t.me/vibekitvn_bot')}`;
    if (interactive) {
