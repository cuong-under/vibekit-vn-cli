...
import { ensureDir, writeFile } from '../installer/file-ops.js';
import {
  loadPlatforms,
  parseToolsOption,
  validateTools,
...
import { resolveProjectPath, toProjectRelativePath } from '../installer/path-utils.js';
...
export async function installFull(options = {}) {
  ...
  if (!bundleExists(targetDir)) {
    ...
    const msg = lic
      ? `Skill content chưa được tải về. Chạy lại: ${ui.color.cyan('vibekitvn activate <KEY>')} (${maskLicenseKey(lic.license_key)})`
    ...
    for (const m of chosenModules) {
      ...
      for (const p of prompts) {
        ...
        if (interactive) {
          ...
          answers[p.key] = applyResult(p, value, answers);
        } else {
          answers[p.key] = applyResult(p, p.default, answers);
        }
      }
    }
  }

  normalizeAnswerPaths(targetDir, answers);
  ...
  if (interactive && actionType !== 'quick-update') {
    const summary = [
      `Thư mục:        ${targetDir}`,
      `CLI installer:  v${PKG.version}`,
      `Skill content:  v${contentVersion}`,
  ...
  const runSteps = async () => {
    materializeProjectBundle(targetDir, bundleRoot, skills);
    const outputFolder = answers.output_folder || '_vibekitvn-output';
    ensureDir(resolveProjectPath(targetDir, outputFolder));

    // Module-defined extra directories
    for (const m of chosenModules) {
      for (const dir of loadModuleDirectories(m, answers)) {
        ensureDir(resolveProjectPath(targetDir, dir));
      }
    }
    ...
    generateConfigs(targetDir, skills, platforms, {
      ...
      outputFolder,
      answers,
      modules: chosenModules.map((m) => m.id),
    });
...
function normalizeAnswerPaths(targetDir, answers) {
  answers.output_folder = toProjectRelativePath(
    targetDir,
    answers.output_folder || '_vibekitvn-output'
  );

  const defaults = {
    planning_artifacts: `${answers.output_folder}/planning-artifacts`,
    implementation_artifacts: `${answers.output_folder}/implementation-artifacts`,
    project_knowledge: 'docs',
  };

  for (const [key, fallback] of Object.entries(defaults)) {
    answers[key] = toProjectRelativePath(targetDir, answers[key] || fallback, fallback);
  }

  for (const [key, value] of Object.entries(answers)) {
    if (typeof value !== 'string' || !isPathAnswerKey(key)) continue;
    answers[key] = toProjectRelativePath(targetDir, value, value);
  }
}

function isPathAnswerKey(key) {
  return /(?:^|_)(?:folder|dir|path|location|artifacts|knowledge)(?:_|$)/.test(key);
}
...
function pickModulesFromOptions(allModules, options) {
  ...
  const aliases = { vbk: 'vbk-dev', 'vbk-ky-nang': 'vbk-dev' };
  const normalized = wanted.map((id) => aliases[id] || id);
  return allModules.filter((m) => normalized.includes(m.id));
}
...
function writeAnswerSnapshot(targetDir, answers, modules, platforms, version) {
  ...
  writeFile(path.join(targetDir, '_vibekitvn', 'vbk', 'config.yaml'), yaml.dump(snapshot));
  for (const m of modules) {
    writeFile(path.join(targetDir, '_vibekitvn', m.id, 'config.yaml'), yaml.dump(snapshot));
  }
}
