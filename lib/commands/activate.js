...
import { verifyLicense, deactivateLicense } from '../api.js';
import { writeLicense, deleteLicense } from '../license-file.js';
...
const KEY_RE = /^VBK-(LIFETIME|FOUNDING|ENT|V2|STANDARD|COMBO)-[0-9A-Z]{4}-[0-9A-Z]{4}-[0-9A-Z]{4}$/i;
...
export async function activateCommand(licenseKeyArg, opts = {}) {
  ...
  if (!licenseKey) {
    const ans = await p.text({
      ...
      placeholder: 'VBK-COMBO-XXXX-YYYY-ZZZZ',
  ...
  // Download + extract — nếu fail, rollback activation để không burn slot (P1-1)
  const dl = p.spinner();
  ...
  try {
    ...
    // Rollback: release activation slot + xoá license file local
    const rb = p.spinner();
    rb.start('Đang rollback activation…');
    try {
      ...
      deleteLicense();
      rb.stop(pc.dim('  rollback OK — slot đã được giải phóng'));
    } catch (rbErr) {
      rb.stop(pc.yellow('  rollback fail — chạy `vibekitvn deactivate` thủ công sau'));
    }
    p.cancel(
      `Lỗi: ${e.message}\nThử chạy lại \`vibekitvn activate <KEY>\` (${maskLicenseKey(licenseKey)}).`
    );
    process.exit(1);
  }

  // Stamp every .md skill with a license-bound watermark for forensic
  // traceability if leaked. Best-effort; failures must not block install.
  try {
    ...
    if (process.env.VIBEKITVN_DEBUG) {
      console.log(pc.dim(`  watermark: stamped ${wm.stamped}/${wm.total} skill files`));
    }
  } catch (e) {
    if (process.env.VIBEKITVN_DEBUG) {
...
function formatVerifyError(code, message) {
  switch (code) {
    ...
    case 'CLI_TOO_OLD':
      return (
        message ||
        'CLI quá cũ. Cập nhật: `npm i -g vibekitvn@latest` rồi chạy lại activate.'
      );
    case 'LICENSE_EXPIRED':
      return 'License đã hết hạn. Liên hệ @vibekitvn_bot → /support để gia hạn.';
    default:
