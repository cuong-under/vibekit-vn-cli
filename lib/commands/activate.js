// lib/commands/activate.js — `vibekit-vn activate <KEY>`
...
export async function activateCommand(licenseKeyArg, opts = {}) {
  ...
  try {
    ...
    p.cancel(`Lỗi: ${e.message}\nThử chạy lại \`vibekit-vn activate\`.`);
    process.exit(1);
  }

  // Auto-sync to AI IDEs (commander maps --no-sync → opts.sync=false)
  if (opts.sync !== false) {
    const sync = p.spinner();
    sync.start('Đang sync skill vào AI IDE…');
    try {
      ...
      sync.stop(pc.yellow('⚠ Sync skip — chạy `vibekit-vn install` thủ công'));
    }
  }
...
function formatVerifyError(code, message) {
  switch (code) {
    ...
    case 'MAX_MACHINES':
      return (
        message ||
        'Đã đạt giới hạn 2 máy. Dùng `vibekit-vn deactivate` trên máy cũ để giải phóng slot.'
      );
    case 'RATE_LIMITED':
