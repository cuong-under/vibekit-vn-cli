...
export function discoverSkills(bundleRoot) {
  ...
  if (manifest?.skills) {
    for (const s of manifest.skills) {
      moduleByName.set(s.name, MODULE_TO_CATEGORY[s.module] || s.module || 'unknown');
    }
  }
...
function parseSkillFile(skillPath, skillDir, manifestModule) {
  try {
    ...
    return {
      ...
      category: fm.category || manifestModule || 'unknown', // domain category from frontmatter
      module: manifestModule || fm.module || 'unknown',     // 'core' or 'vbk' (used by install filter)
