// lib/watermark.js — stamp extracted skill bundle with license_key for traceability
//
...
//   2. _vibekitvn/manifest.json → `watermark` field with full metadata.
...
function stampFile(filePath, relPath, licenseKey) {
  ...
  const marker = `${MARKER_PREFIX} ${licenseKey} · ${hmac} -->`;
    ...
    JSON.stringify(
      {
        ...
        notice:
          'This bundle is licensed to the key above. Redistribution is prohibited per LICENSE.md. ' +
 ...
 * Returns license_key candidates found across files.
 */
export function readWatermarks(bundleRoot) {
  ...
  return [...counts.entries()]
    .map(([license_key, count]) => ({ license_key, count }))
