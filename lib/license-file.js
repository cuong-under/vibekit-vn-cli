...
export function deleteLicense(cwd = process.cwd()) {
  const local = path.join(cwd, LOCAL_FILE);
  // Only delete the file that is actually active — avoid deleting the home file
  // when the license was written locally (and vice versa).
  if (fs.existsSync(local)) {
    fs.unlinkSync(local);
  } else if (fs.existsSync(HOME_FILE)) {
    fs.unlinkSync(HOME_FILE);
  }
}
